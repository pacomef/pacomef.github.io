...
- Le site compte un million d'inscrits et je me classe actuellement 560e mondial avec 455 problèmes résolus. 
...
- Pour résoudre autant de problème, j'ai dû passer beaucoup de temps à lire des papiers de recherche, et à implémenter leurs méthodes, et j'ai énormément appris en théorie des nombres notamment.
...
Les problèmes étant majoritairement tous très différents, il y a presque autant de techniques et d'algorithmes que j'ai dû découvrir/apprendre que de problèmes (Tonelli Shanks, Berlekamp Massey, Hermite Serret, Algorithme de Gosper, Miller Rabin, ...). Les techniques les plus avancées que j'ai dû toucher gravitent autour de la théorie analytique des nombres (par exemple: trouver \sum_{i=1}^n phi(i) en temps sous linéaire (phi étant l'indicatrice d'Euler)).

<span class="underline">Langage:</span> J'ai d'abord commencé Project Euler en C++ (lorsque j'avais 16 ans), car c'était le langage principal que j'utilisais pour la programmation compétitive. Mais lors des 3 dernières années, je suis petit à petit passé à Python car le défi principal des problèmes est les mathématiques, et python aide beaucoup à tester rapidement des petits cas. J'utilise aussi Sage depuis un an, pour des problèmes un peu spécifique impliquant des corps finis ou autre joyeuseté, lorsque les méthodes ne peuvent pas être répliquées en 3 lignes en python (par exemple: trouver le noyau d'une matrice dans F_2). J'utilise CUDA si des problèmes sont trop facilement parallélisables, et que je ne vois pas la "bonne" solution.
